      ...
      ((typep ast 'mal-data-list)
        (if (zerop (length (value ast)))
          ...
          (let ((str (get-str (first (value ast)))) (lis ast))
            (cond
              ((equal str "eval") (setf ast (second (value lis))))
              ...
              ((equal str "do") (loop for i in (butlast (value lis)) do (mal-eval i env))
                                (setf ast (last (value lis))));(car (last (loop for i in (cdr (value lis)) collecting (mal-EVAL i env)))))
